...
public class OnTestClickListener implements OnClickListener {
	...
	private static final int TEST_CNT = 15;
	...
	public OnTestClickListener(String extra,TextView _tv, ContentResolver _cr) {
		this.extra=extra;
		mTextView = _tv;
		mContentResolver = _cr;
		mContentValues = initTestValues();
	...
	private class Task extends AsyncTask<Void, String, Void> {
		...
		private void testQueryStar() {
			Cursor resultCursor = mContentResolver.query(mUri, null,
					"*", null, null);

			if(resultCursor.getCount()==0){
				publishProgress("Null Stored\n");
			}
			else {
				do {
